...
export default function Promotions() {
  ...
  const { availableProducts, productsLoading, productsError } =
  ...
  const handleSubmit = async (e) => {
    ...
    if (!res.ok) {
      ...
      setFormError(errorData?.detail || "Error al guardar el descuento.");
      setLoading(false);
      return;
    }
    ...
    <div className="admin-form-card" style={{ maxWidth: 700, margin: "2rem auto" }}>
      ...
      <form onSubmit={handleSubmit} className="admin-form">
        ...
        <div className="admin-form-field">
          ...
          <div className="select-wrapper">
            <Select
              ...
              onChange={(selected) => {
                const updatedItems = selected.map((p) => {
                  const existing = form.productos.find((fp) => fp.value === p.value);
                  return form.tipo === "PRECIO_FIJO"
                    ? { ...p, cantidad: existing?.cantidad || 1 }
                    : { ...p };
                });
                setForm({ ...form, productos: updatedItems });
              }}
        ...
        <div className="admin-form-actions">
          <button type="submit" className="admin-form-btn admin-form-btn--primary" disabled={loading || !!formError}>
